...
SELECT 
    ...
    cs.start_time, 
    cs.end_time, 
    (c.capacity - COALESCE(rc.reg_count, 0)) AS available_spots
FROM 
    classes c
JOIN 
...
LEFT JOIN 
    (SELECT 
     ...
     FROM 
        class_attendance  
     GROUP BY 
        schedule_id) rc ON rc.schedule_id = cs.schedule_id;
...
SELECT 
    AVG(classes_per_member) AS average_classes_per_member
FROM 
    (
        SELECT 
            member_id, 
            COUNT(*) AS classes_per_member
        FROM 
            class_attendance
        GROUP BY 
            member_id
