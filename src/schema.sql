...
CREATE TABLE locations (  
    location_id INT CHECK(length(location_id) = 1) PRIMARY KEY,  
    name VARCHAR(255),  
    address TEXT,  
...
-- `members` table  
CREATE TABLE members (  
    member_id INT CHECK(length(member_id) = 1) PRIMARY KEY,  
    first_name VARCHAR(255),  
    last_name VARCHAR(255),  
    email VARCHAR(255),  
    phone_number VARCHAR(20) CHECK(length(phone_number) = 8),  
    date_of_birth DATE CHECK(length(date_of_birth) = 10),  
    join_date DATE CHECK(length(join_date) = 10),  
...
-- `staff` table  
CREATE TABLE staff (  
    staff_id INT CHECK(length(staff_id) = 1) PRIMARY KEY,
    first_name VARCHAR(255),
    last_name VARCHAR(255),
    email VARCHAR(255),
    phone_number VARCHAR(20) CHECK(length(phone_number) = 8),  
    position ENUM('Trainer', 'Manager', 'Receptionist', 'Maintenance'),
    hire_date DATE CHECK(length(hire_date) = 10),
    location_id INT CHECK(length(location_id) = 1),
    FOREIGN KEY (location_id) REFERENCES locations(location_id)
);  
  
-- `equipment` table  
CREATE TABLE equipment (  
    equipment_id INT CHECK(length(equipment_id) = 1) PRIMARY KEY,  
    name VARCHAR(255),  
...
-- `classes` table  
CREATE TABLE classes (  
    class_id INT CHECK(length(class_id) = 1) PRIMARY KEY,  
    name VARCHAR(255),  
    description TEXT,  
    capacity INT CHECK(length(capacity) = 2),  
    duration INT CHECK(length(duration) = 2),  
    location_id INT CHECK(length(location_id) = 1),  
    FOREIGN KEY (location_id) REFERENCES locations(location_id)  
);  
...
CREATE TABLE class_schedule (  
    schedule_id INT CHECK(length(schedule_id) = 1) PRIMARY KEY,  
    class_id INT CHECK(length(class_id) = 1),  
    staff_id INT CHECK(length(staff_id) = 1),  
...
-- `memberships` table  
CREATE TABLE memberships (  
    membership_id INT CHECK(length(membership_id) = 1) PRIMARY KEY,  
    member_id INT CHECK(length(member_id) = 1),  
    type VARCHAR(255),  
...
CREATE TABLE attendance (  
    attendance_id INT CHECK(length(attendance_id) = 1) PRIMARY KEY,  
    member_id INT CHECK(length(member_id) = 1),  
    location_id INT CHECK(length(location_id) = 1),  
...
CREATE TABLE class_attendance (  
    class_attendance_id INT CHECK(length(class_attendance_id) = 1) PRIMARY KEY,  
    schedule_id INT CHECK(length(schedule_id) = 1),  
    member_id INT CHECK(length(member_id) = 1),  
...
CREATE TABLE payments (  
    payment_id INT CHECK(length(payment_id) = 1) PRIMARY KEY,  
    member_id INT CHECK(length(member_id) = 1),  
    amount DECIMAL(10, 2),  
...
CREATE TABLE personal_training_sessions (  
    session_id INT CHECK(length(session_id) = 1) PRIMARY KEY,  
    member_id INT CHECK(length(member_id) = 1),  
    staff_id INT CHECK(length(staff_id) = 1),  
...
CREATE TABLE member_health_metrics (  
    metric_id INT CHECK(length(metric_id) = 1) PRIMARY KEY,  
    member_id INT CHECK(length(member_id) = 1),  
...
CREATE TABLE equipment_maintenance_log ( 
    log_id INT CHECK(length(log_id) = 1) NOT NULL PRIMARY KEY,  
    equipment_id INT CHECK(length(equipment_id) = 1),
    maintenance_date DATE CHECK(length(maintenance) = 10),
    description TEXT,
    staff_id INT CHECK(length(staff_id) = 1),
